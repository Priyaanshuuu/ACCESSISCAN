...
export function DashboardClient() {
  ...
  async function loadDashboard() {
    ...
    await Promise.all([
      getPaidResource(currentPlan, "/api/schedules", { schedules: [] as Schedule[] })
  ...
  async function updateSchedule(url: string, method: "POST" | "PATCH", body: object) {
    if (!hasPaidPlan(plan)) { setBillingOpen(true); return; }
    setScheduleError("");
    ...
    <main id="main-content" className="workspace page-shell flex-1 text-[#19251f]">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="flex flex-col gap-4 border-b border-[#d7dfd5] pb-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap gap-3"><Button variant="outline" onClick={() => setBillingOpen(true)} type="button">{plan === "FREE" ? "Upgrade plan" : `${plan} plan`}</Button><Link href="/" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#19382d] px-4 text-sm font-medium text-white hover:bg-[#285342]">New scan <ArrowUpRight aria-hidden="true" className="size-4" /></Link></div>
        </header>
